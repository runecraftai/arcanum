...
import { createFighterAgent, createFighterAgentWithOptions } from "./index"
...
describe("createFighterAgent", () => {
  it("is a callable factory", () => {
    expect(typeof createFighterAgent).toBe("function")
  })

  it("has mode primary", () => {
    expect(createFighterAgent.mode).toBe("primary")
  })
  ...
  it("allows call_guild_agent tool for delegation", () => {
    const config = createFighterAgent("claude-sonnet-4")
  ...
  it("PostExecutionReview references runtime-owned Cleric/Paladin review", () => {
    const config = createFighterAgent("claude-sonnet-4")
    const prompt = config.prompt as string
  ...
  it("verification protocol mentions Ranger output inspection", () => {
    const config = createFighterAgent("claude-sonnet-4")
    const prompt = config.prompt as string
    const verificationSection = prompt.slice(prompt.indexOf("<Verification>"), prompt.indexOf("</Verification>"))
    expect(verificationSection).toContain("Ranger")
  ...
  it("createFighterAgentWithOptions accepts resolved continuation config without changing prompt shape", () => {
    const config = createFighterAgentWithOptions(
      "claude-sonnet-4",
      new Set(),
      {
  ...
  it("createFighterAgentWithOptions includes CategoryRouting section when categories with patterns provided", () => {
    const config = createFighterAgentWithOptions(
      "claude-sonnet-4",
      new Set(),
      undefined,
    ...
    expect(config.prompt).toContain("ranger-frontend")
    expect(config.prompt).toContain("*.tsx")
  })

  it("createFighterAgentWithOptions omits CategoryRouting section when no categories provided", () => {
    const config = createFighterAgentWithOptions("claude-sonnet-4")
  ...
  it("createFighterAgentWithOptions includes manual-only CategoryRouting when categories have no patterns", () => {
    const config = createFighterAgentWithOptions(
      "claude-sonnet-4",
      new Set(),
      undefined,
    ...
    expect(config.prompt).toContain("<CategoryRouting>")
    expect(config.prompt).toContain("ranger-backend")
  ...
  it("createFighterAgentWithOptions marks no-wizard categories as manual-only and never auto-routed from files", () => {
    const config = createFighterAgentWithOptions(
      "claude-sonnet-4",
      new Set(),
      undefined,
  ...
  it("createFighterAgentWithOptions preserves config-order precedence for overlapping category patterns", () => {
    const config = createFighterAgentWithOptions(
      "claude-sonnet-4",
      new Set(),
      undefined,
