 ...
 * Regenerates if the cached fingerprint was generated by a different Guild version.
 * Fire-and-forget: errors are logged but never thrown.
 */
export function getOrCreateFingerprint(directory: string): ProjectFingerprint | null {
  try {
    ...
    if (existing) {
      const currentVersion = getGuildVersion()
      if (existing.guildVersion === currentVersion) {
        return existing
      }
      debug("[analytics] Fingerprint version mismatch — regenerating", {
        cached: existing.guildVersion ?? "none",
