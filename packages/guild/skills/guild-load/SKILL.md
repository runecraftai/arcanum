...
Read the canonical `.guild/` state in the documented loading order, surface blockers and open follow-ups, and stop before touching anything. Prefer the smallest useful context set and respect the token budget.
...
5. If `.guild/context/project.md` is absent, note the absence — the project may not be initialized. Run `guild-init` first.
