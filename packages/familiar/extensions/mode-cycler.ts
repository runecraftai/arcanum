// ABOUTME: Cycles operational modes (NORMAL/PLAN) via Shift+Tab.
// ABOUTME: Simplified for Runecraft — only NORMAL and PLAN modes.
...
import { FAMILIAR_PLAN_PROMPT, buildFamiliarNormalPrompt } from "./lib/mode-prompts-familiar.ts";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "set_mode",
		label: "Set Mode",
		description: "Switch between NORMAL (orchestrator) and PLAN (spec-driven) modes.",
		parameters: Type.Object({
			mode: Type.String({ description: "Target mode: NORMAL or PLAN" }),
	...
	pi.on("before_agent_start", async (_event, _ctx) => {
		if (currentMode === "NORMAL") {
			return { systemPrompt: buildFamiliarNormalPrompt() };
		}
		if (currentMode === "PLAN") return { systemPrompt: FAMILIAR_PLAN_PROMPT };
		return {};
	});
