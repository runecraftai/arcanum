...
**Status**: ✅ Complete — all 27 tasks done (T01..T27)
...
**Done when**:
- [x] The doc has a "Skill anatomy" section that names every section, describes its purpose, and states the ≥ 3 Rationalizations rule for workflow-shaped skills.
- [x] The doc's "Verification" subsection of the anatomy states the rule "'seems right' is not evidence."
- [x] The bundled-skills table lists 15 entries including `guild-configurator` and `guild-recon`.
- [x] Cross-references to related docs (e.g., authoring tips) point to the new anatomy.
...
**Done when**:
- [x] File contains the headings `## Overview`, `## When to Use`, `## Process`, `## Rationalizations`, `## Red Flags`, `## Verification`.
- [x] Rationalizations table has ≥ 3 rows naming common skip-rationalizations and rebuttals.
- [x] Verification gate names specific evidence (test output, lint output, `tasks.md` updates).
- [x] No behaviour change beyond structural expansion.
...
**Done when**:
- [x] Anatomy headings present (`## Overview`, `## When to Use`, `## Process`, `## Rationalizations`, `## Red Flags`, `## Verification`).
- [x] Process includes two checks: per-task (existing) and project-wide (new — reads `knowledge/definition-of-done.md`).
- [x] Process states: when the project-wide file is absent, log a note in `plans/<slug>/notes.md` and continue with per-task only.
- [x] Verification gate requires evidence for BOTH criteria when both are present.
- [x] Rationalizations table has ≥ 3 rows including "the build is green, that's enough" and "I'll add tests later" style rebuttals.
...
**Done when**:
- [x] Anatomy headings present.
- [x] Rationalizations table has ≥ 3 rows.
- [x] Red Flags list ≥ 2 items (e.g., "reviewer signs off without running tests", "reviewer paraphrases spec acceptance criteria").
- [x] Verification gate names specific evidence (review status in `state.md`, findings in `notes.md`).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process includes a "DoD gate" step that reads `knowledge/definition-of-done.md` and confirms every bar item is met.
- [x] When the project-wide file is absent, the skill logs a note in `plans/<slug>/notes.md` and continues with per-task checks only.
- [x] Verification gate requires both per-task and project-wide evidence.
- [x] Rationalizations table has ≥ 3 rows including "it's a small change, no need for a checklist" and "the merge button is right there" style rebuttals.
...
**Done when**:
- [x] Anatomy headings present.
- [x] Rationalizations table has ≥ 3 rows including "this is internal, no need for input validation" and "the secret is in a private repo" style rebuttals.
- [x] Red Flags list ≥ 2 items (e.g., "secrets present in diff", "trust boundary crossed without validation").
- [x] Verification gate names specific evidence (security findings in `notes.md`, updated `gotchas.md` if new pitfalls).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps to produce a `spec.md` (problem statement, goals, out of scope, user stories, edge cases, success criteria).
- [x] Rationalizations table has ≥ 3 rows including "I already know what to build" and "the spec is overhead" style rebuttals.
- [x] Verification gate names specific evidence (the `spec.md` file with all required sections).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps to produce a `tasks.md` (atomic tasks with verification criteria, dependency ordering).
- [x] Rationalizations table has ≥ 3 rows including "I can hold the whole plan in my head" and "tasks are bureaucracy" style rebuttals.
- [x] Verification gate names specific evidence (`tasks.md` with verification criteria per task).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists the artifact-scope rule (small = single plan doc; medium = full plan; large = spec + design + tasks + extras).
- [x] Rationalizations table has ≥ 3 rows including "it's clearly small, no need to declare scope" and "I'll add tasks later" style rebuttals.
- [x] Verification gate names specific evidence (the chosen artifact set with file paths).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process includes a "Scaffold DoD file" step with the template content (header comment, empty `## Project-wide standing bar` section, `## Per-task criteria` pointer).
- [x] Behavior rules include: "If the file already exists, do not touch it."
- [x] Behavior rules preserve the existing context scaffold behaviour (no regression).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps for human-to-agent and agent-to-agent handoff.
- [x] Rationalizations table has ≥ 3 rows including "I'll just tell them in the next turn" and "the handoff is overhead" style rebuttals.
- [x] Verification gate names specific evidence (`context/handoff.md` and `plans/<slug>/state.md` updated).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps to load existing project context into a new session.
- [x] Red Flags list ≥ 2 items (e.g., "context is stale", "legacy `.specs/` content read but not migrated").
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps for external documentation lookups with source citations.
- [x] Verification gate names specific evidence (sources cited inline; answer traceable to a specific URL or doc section).
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process lists numbered steps for capturing reusable lessons into `.guild/knowledge/decisions.md` or `gotchas.md`.
- [x] Red Flags list ≥ 2 items (e.g., "lesson is too specific to one change", "lesson is captured in `notes.md` but never promoted").
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process describes the configuration workflow.
...
**Done when**:
- [x] Anatomy headings present.
- [x] Process describes the reconnaissance workflow.
...
**Done when**:
- [x] File exists.
- [x] Contains the sections "Endorsed patterns", "Anti-patterns", "Bard is a lead".
- [x] Endorsed patterns section covers: direct invocation, single-persona command, parallel fan-out with merge, sequential pipeline (user is orchestrator), research isolation.
- [x] Anti-patterns section covers: router persona, persona-calls-persona, sequential orchestrator that paraphrases, deep persona trees.
- [x] Each anti-pattern has a "How this would look in Guild" subsection naming specific primitives.
...
**Done when**:
- [x] `docs/README.md` "Customize how agents think and act" table has a row pointing to `orchestration.md`.
- [x] `docs/agents.md` "See also" section has a link to `orchestration.md`.
- [x] `docs/custom-agents.md` "See also" section has a link to `orchestration.md`.
- [x] All links resolve (no 404s in the rendered docs).
...
**Done when**:
- [x] Knowledge table includes `definition-of-done.md`.
- [x] Canonical loading order includes `definition-of-done.md` in the correct position.
- [x] Boundary table includes the new DoD row.
- [x] `knowledge/index.md` (if it exists) is also updated to mention the new file; otherwise this requirement is noted for the next time the file is created. (`.guild/knowledge/` does not exist yet; deferred to first init.)
...
**Done when**:
- [x] T10 deliverable verified.
- [x] A note in `guild-init`'s Verification section explicitly states: "Re-running guild-init does not modify an existing `.guild/knowledge/definition-of-done.md`."
...
**Done when**:
- [x] T03 deliverable verified.
- [x] Process and Verification gate explicitly state: when `knowledge/definition-of-done.md` is absent, log a note in `plans/<slug>/notes.md` and continue with per-task only.
...
**Done when**:
- [x] T05 deliverable verified.
- [x] Process explicitly orders: per-task checks → DoD gate → ship declaration.
- [x] When the project-wide file is absent, the skill logs a note and continues.
...
**Done when**:
- [x] `definition-of-done.md` exists with all six parts above.
- [x] The page contains the phrase "project-wide standing bar".
- [x] `docs/README.md` has a "Project state" section linking to the DoD page.
- [x] The page links to `.guild/architecture.md` for the canonical loading order.
...
- [x] `bunx turbo lint` passes for `packages/guild`. (No `lint` script defined; `bun run typecheck` was run as the closest equivalent.)
- [x] `bunx turbo typecheck` passes for `packages/guild`. (`bun run typecheck` clean: 0 errors.)
...
**Done when**:
- [x] All 15 skills contain the headings `## Process`, `## Rationalizations`, `## Red Flags`, `## Verification` (Rationalizations may be omitted only on skills where no skip-rationalization applies; this exception is documented per-skill).
- [x] The workflow-shaped skills (execute, verify, review, ship, security, plan, spec, scope, init, handoff) have ≥ 3 Rationalizations rows each. (All ten have 7+ rows.)
...
**Done when**:
- [x] `bun test` in `packages/guild` passes. (Baseline 2038 pass / 31 fail is identical before and after this feature; no regression introduced. The 31 failures are pre-existing, not caused by this work.)
- [x] If any test broke, the fix is recorded in `plans/<slug>/notes.md` (or in a follow-up task) with a rationale. (No test broke; nothing to record.)
...
**Done when**:
- [x] `git diff --stat packages/spells/` returns empty.
- [x] `git diff --stat packages/summon/` returns empty.
- [x] `git diff --stat packages/familiar/ packages/grimoire/ packages/spawn/` returns empty (these are unrelated to this feature).
